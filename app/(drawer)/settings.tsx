...
import { Share, LogOut, Bell, Moon, User, Shield, HelpCircle, MessageSquare, UserX } from 'lucide-react-native';
...
export default function Settings() {
  ...
  // 客服权限检查
  const isServiceAgent = agent?.permissions.includes('console_access') || agent?.permissions.includes('agent_settings') || false;
    ...
    ...(isServiceAgent && !isAdmin ? [
      {
        ...
        items: [
          ...
          ...(agent?.permissions.includes('blacklist_management') ? [{
            icon: <UserX size={22} color={COLORS.primary} />,
            label: '黑名单管理',
            onPress: () => {
              router.push('../admin/blacklist');
          ...
          ...(agent?.permissions.includes('share_links') ? [{
